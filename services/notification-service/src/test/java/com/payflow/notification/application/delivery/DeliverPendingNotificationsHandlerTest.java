...
class DeliverPendingNotificationsHandlerTest {
    ...
    private final DeliverPendingNotificationsHandler handler = new DeliverPendingNotificationsHandler(
            store, email, properties, metrics, Clock.fixed(NOW, ZoneOffset.UTC));

    @Test
    void deliversAfterClaimAndMarksSent() {
        var claimed = claimed();
        when(store.claim(anyString(), eq(NOW), eq(properties.lease()), eq(5), eq(10)))
                .thenReturn(new NotificationClaimBatch(List.of(claimed), 0));
        when(store.oldestPendingAgeSeconds(NOW)).thenReturn(2.0);
    ...
    void providerExceptionBecomesStableFailureCodeWithoutLeakingMessage() {
        var claimed = claimed();
        when(store.claim(anyString(), any(), any(), eq(5), eq(10)))
                .thenReturn(new NotificationClaimBatch(List.of(claimed), 0));
