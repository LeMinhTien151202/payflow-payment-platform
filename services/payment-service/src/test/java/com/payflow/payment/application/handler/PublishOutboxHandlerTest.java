...
class PublishOutboxHandlerTest {
    ...
    private static final String OWNER = "payment-service:publisher-1";
    ...
    @Test
    @DisplayName("a failure releases later events of the same aggregate without sending them out of order")
    ...
    @Test
    @DisplayName("reclaimed rows and a lost conditional mark are visible in the batch result")
    ...
    @Test
    @DisplayName("stored error detail is always capped at the database column width")
    void capsSafeError() {
    ...
    private PublishOutboxHandler handler(int maxAttempts) {
        return new PublishOutboxHandler(
                store,
                transport,
                new OutboxPublishPolicy(100, Duration.ofSeconds(120), maxAttempts, Duration.ofSeconds(300)),
    ...
    private static final class FakeStore implements OutboxLeaseStore {
        ...
        @Override
        public boolean markRetry(UUID eventId, String owner, Instant nextAttemptAt, String safeError) {
