...
public final class ProjectionEventParser{
 ...
 public ProjectionEvent parse(String key,String payload){
  var node=json.readTree(payload);String aggregate=node.required("aggregateId").stringValue();
  if(key==null||!key.equals(aggregate))throw new IllegalArgumentException("Kafka key must equal aggregateId");
  int version=node.required("eventVersion").asInt();String eventType=node.required("eventType").stringValue();
  ...
  return new ProjectionEvent(UUID.fromString(node.required("eventId").stringValue()),eventType,
    version,aggregate,Instant.parse(node.required("occurredAt").stringValue()),node.required("data"),payload);
 }
}
