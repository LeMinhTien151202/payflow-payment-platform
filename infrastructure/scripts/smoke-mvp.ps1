...
function Invoke-Compose {
    ...
    & docker compose --env-file $EnvFile --profile mvp @Arguments
...
function Invoke-PsqlScalar {
    ...
    $output = & docker compose --env-file $EnvFile --profile mvp exec -T postgres `
...
try {
    ...
    Write-Host 'Validating the resolved MVP Compose model...'
    ...
    $healthTargets = [ordered]@{
        ...
        'account-ledger-service'= "http://localhost:$(Require-EnvironmentValue 'PAYFLOW_ACCOUNT_LEDGER_PORT')/actuator/health/readiness"
    ...
    $balanceBefore = Invoke-PsqlScalar 'payflow_account_ledger' `
    ...
    if ($balanceBefore -ne '1000000.0000|0.0000') {
        throw "The happy-path account is not pristine (actual $balanceBefore). Run 'docker compose --profile mvp down -v' only if deleting disposable local data is intended, then start the MVP again."
    }
    ...
    $requestBody = [ordered]@{
        ...
        amount             = 500000
        currency           = 'VND'
        description        = 'PayFlow local MVP smoke payment'
        metadata           = @{ scenario = 'phase-1b-happy-path' }
    ...
    $balanceAfter = Invoke-PsqlScalar 'payflow_account_ledger' `
        "select available_balance::text || '|' || reserved_balance::text from account.accounts where id = '$HappySourceAccountId';"
    $reservation = Invoke-PsqlScalar 'payflow_account_ledger' `
        "select status from account.balance_reservations where payment_id = '$paymentId';"
    $journal = Invoke-PsqlScalar 'payflow_account_ledger' `
    ...
    Write-Host ''
    Write-Host "MVP SMOKE PASSED: $paymentId" -ForegroundColor Green
